...
% N_u is the number of inputs
N_u = size(data, 1);
% n is an N_u dimensional vector of ones
n = ones(N_u, 1);
...
for epoch = 1:MAX_EPOCHS
    ...
    for p = 1:size(data, 2)
        ...
        v = dot(w, u);
        % Apply online Hebb Learning rule with subtractive normalization
        delta_w = eta * (v * u - (v * dot(n, u) * n)/N_u);
        %page 306
        w = w + delta_w;
        W = cat(2, W, w);
    end
...
saveas(gcf, 'Assignment3/Results/Plot_P1_weight_eigenvector.png')
...
saveas(gcf, 'Assignment3/Results/Plot_P2_w(1).png')
...
saveas(gcf, 'Assignment3/Results/Plot_P2_w(2).png')
...
saveas(gcf, 'Assignment3/Results/Plot_P2_norm_w.png')

% Save evolution of w (matrix W) as .mat
save('Assignment3/Results/W.mat', 'W');